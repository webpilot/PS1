...
$ENV:PSversion = $host.Version.ToString()

#Set console colors
#reset colors to normal just in-case
[console]::ResetColor()

#Set dir to users home
Set-Location $ENV:HOMEPATH\"Google Drive\School\Powershell"

function prompt {
    # Set Window Title
    $host.UI.RawUI.WindowTitle = "PowerShell ver $ENV:PSversion - $(Get-Location)"
    ...
    if ($IsAdmin) {       
        Write-Host "(admin) #" -NoNew -Fore Gray
        return " "
    }
    else {       
        Write-Host ">" -NoNew -Fore Gray
        return " "
    }
 }
